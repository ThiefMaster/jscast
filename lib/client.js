...
function PlaybackClient(manager, req, res, id) {
    ...
    this.bytesSent = 0;
    this.wantsMetadata = this.manager.metaInterval && (req.headers['icy-metadata'] === '1');
    ...
    if(manager.metadata.bitrate) {
        headers['icy-br'] = manager.metadata.bitrate;
    }
...
_.extend(PlaybackClient.prototype, {
    ...
    _sendMetadata: function _sendMetadata() {
        ...
        if(metadata === this.lastMetadata || !this.manager.metadata.song) {
...
function ClientManager(jscast) {
    this.jscast = jscast;
    this.clients = {};
    this.clientCount = 0;
    this.clientPeak = 0;
    this.metaInterval = this.jscast.settings.stream.metaInterval;
    this.metadata = this.jscast.metadata;

    this.jscast.on('audio', this.broadcast.bind(this));
}
...
_.extend(ClientManager.prototype, {
    newClient: function newClient(req, res) {
        ...
        this.clientCount++;
        if(this.clientCount > this.clientPeak) {
            this.clientPeak = this.clientCount;
        }
    ...
    getStats: function getStats() {
        var unique = _.uniq(_.map(this.clients, function(client) {
            return client.req.ip;
        })).length;

        return {
            total: this.clientCount,
            unique: unique,
            peak: this.clientPeak
        };
    }
});
